...
var Unidadperiodo2 = require('../models/unidadperiodo2');
...
exports.deleteUnidadperiodo2 = function(req, res, next){
   
    Bitacora.create({email: req.params.userID ,permiso:'Elimina',accion:'Elimina Edificio '});
    Unidadperiodo2.findByIdAndRemove({ _id: req.params.recordID  }, function(err, todo) {
        res.json(todo);
    });
}


exports.creaUnidadperiodo22s = function(req, res, next){
   
 
    Bitacora.create(req.body.bitacora);
if(req.params.recordID!=='crea')
{
    Unidadperiodo2.findById({ _id: req.params.recordID }, function (err, todo)  {
        if (err) {  res.send(err);  }
        else
        {  
    ...
    Unidadperiodo2.find({idtipounidad        	: req.body.idtipounidad        	,
