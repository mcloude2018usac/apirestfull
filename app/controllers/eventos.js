...
exports.getEvento = function(req, res, next){
    ...
    { 
        if(req.params.id4=='ADMINISTRADOR')
        {
            ...
            select({plantilla:1,idempresa:1,unidad:1,costo:1,fecha:1,impresion:1,nombre:1,fechaini:1,fechafin:1,ubicacion: 1,nomax:1,foto:1,fecha:1}).sort([['createdAt', -1]]).exec(function(err, todos) {
                if (err){  res.send(err);  }
                 res.json(todos);
             });
        }
    ...
    { 
        if(req.params.id3=='eventos' )
        {
            ...
            select({plantilla:1,idempresa:1,unidad:1,costo:1,fecha:1,impresion:1,nombre:1,fechaini:1,fechafin:1,ubicacion: 1,nomax:1,foto:1,fecha:1}).lean().sort([['createdAt', -1]]).exec(function(err, todos) {
                if (err){  res.send(err);  }
                 res.json(todos);
